...
// ── الاسم المختصر ولا الكامل؟ الاتنين، كل واحد فى مكانه ──────────────────────
//   • **الأعطال والمتعذرات** → الاسم المختصر («سامى»، «حسن») — زى ما هو مسجّل فى
//     جدول أسماء الفنيين، والتقارير دى مابتلمسهوش خالص.
//   • **أوامر الشغل واستكمال البيانات** → الاسم الكامل («محمد عبدالعزيز طه احمد»)
//     — عن طريق preferFullName/fullNameOf.
// المفتاح المختصر (name) بيتستخدم كمان فى المقارنة والفلترة جوّه النظام، ومابيتعرضش
// فى تقارير أوامر الشغل.

export interface Technician {
  /** الاسم المختصر — بيتعرض فى الأعطال والمتعذرات، وبيتستخدم للمقارنة والفلترة. */
  name: string;
  /** الاسم الكامل — بيتعرض ويتخزّن فى أوامر الشغل واستكمال البيانات. */
  fullName: string;
  /** أى صيغ تانية بيتكتب بيها الاسم فى الملفات. */
  aliases: string[];
}

// المرجع للأسماء هو **ملف أوامر الشغل** (اتأكد من المستخدم). أى صيغة تانية موجودة فى
// ملفات تانية بتتسجّل هنا كـ alias عشان المطابقة تشتغل من أى مصدر، لكن **العرض والتخزين
// بيبقوا بالاسم المعتمد (fullName)** دايماً.
// مثال حقيقى: جدول أسماء الفنيين مكتوب فيه «حسن عبد الفتاح يعقوب» وملف أوامر الشغل
// «حسن عبد الفتاح حموده» — التانى هو الصح، والأول اتساب alias بس.
...
/** الاسم المختصر للفنى (للأعطال والمتعذرات) — أى اسم تانى بيرجع زى ما هو. */
export const shortNameOf = (raw: unknown): string =>
  matchTechnician(raw) ?? String(raw ?? "").trim();
...
/**
 * اسم العرض للفنى: **الاسم المعتمد الكامل** (من ملف أوامر الشغل — المرجع).
 * أى صيغة تانية للاسم (مختصرة زى «سامى»، أو مكتوبة بشكل مختلف زى «حسن عبد الفتاح
 * يعقوب») بتترجم للاسم المعتمد، فالشخص الواحد بيتعرض بنفس الاسم فى كل مكان.
 * أى اسم مش لأى فنى من الخمسة بيرجع زى ما هو — الأسماء الغريبة مابتتلمسش.
 */
export const preferFullName = (registryName: unknown): string => fullNameOf(registryName);
