...
(function () {
  ...
  // نجيب كل الأرقام المطلوبة فى المراجعة اليدوية الشاملة (لحد 12000). المراجعة اليومية بعد التصدير
  // بتتقصّر لدفعة صغيرة (40) عبر maxCount من sf_si_batch — مش عبر limit ده.
  const SI_FETCH_LIMIT = 12000;
  ...
  async function siGetPending() { const r = await siGm({ method: 'GET', url: SF_URL + '/api/line-subscriber-info/pending?limit=' + SI_FETCH_LIMIT, headers: { 'X-DZS-Token': DZS_TOKEN } }); if (!r.ok) { log('❌ SubInfo: فشل جلب القائمة', r.status); return []; } try { return JSON.parse(r.text).phones || []; } catch (e) { return []; } }
