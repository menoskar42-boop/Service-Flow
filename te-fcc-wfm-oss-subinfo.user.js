...
(function () {
  ...
  async function main() {
    ...
    try {
      ...
      else if (host.startsWith('wfm.te.eg')) {
        // تاب «إلغاء الاسناد» بيفتح WorkOrder/faces/Home كمدخل ثم يروح Dispatcher — لو شغّلنا
        // تدفّق التصدير عليه هيخطفه لشاشة Work Order Management ويفتح نافذة Export. فبنقف هنا
        // ونسيب السكربت التانى يكمّل. (العلامة بتتمسح لوحدها من سكربت الإلغاء لما يخلّص.)
        let cancelPending = ''; try { cancelPending = sessionStorage.getItem('sf_wfm_cancel_pending') || ''; } catch (e) {}
        if (cancelPending) { log('⏭ تاب إلغاء إسناد (' + cancelPending + ') — تدفّق تصدير WFM متوقّف على التاب ده.'); return; }
        await runWFM();
      }
  ...
  log('TE FCC + WFM + OSS + SubInfo v3.4.0 loaded on', location.host);
