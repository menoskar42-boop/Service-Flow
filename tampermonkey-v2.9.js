...
(function () {
  ...
  function startLoginCooldown(why) { try { localStorage.setItem('sf_login_cd', String(Date.now() + LOGIN_LOCK_MS)); localStorage.removeItem('sf_fcc_fail'); } catch (e) {} log('⛔ إيقاف الدخول التلقائى ~' + Math.round(LOGIN_LOCK_MS / 60000) + ' دقيقة (' + why + ') — الحساب بيرجع لوحده بعد ~ثلث ساعة.'); }
  function stopAfterSubmit() {
    if (loginRejected()) { startLoginCooldown('الحساب مقفول مؤقتاً / بيانات مرفوضة'); return true; }
    if (loginBlocked()) { startLoginCooldown('LoginException'); return true; }
    return false;
  }
  async function doLogin() {
    ...
    // حد أقصى مرتين محاولة فاشلة (العدّاد مشترك عبر التابات والريلود) قبل ما ندخل تهدئة
    let _fails = 0; try { _fails = Number(localStorage.getItem('sf_fcc_fail') || 0); } catch (e) {}
    if (_fails >= 2) { startLoginCooldown('تجاوزنا مرتين محاولة فاشلة'); return; }
    const bumpFail = () => { _fails++; try { localStorage.setItem('sf_fcc_fail', String(_fails)); } catch (e) {} };
    const okDone = () => { try { localStorage.removeItem('sf_fcc_fail'); localStorage.removeItem('sf_login_cd'); } catch (e) {} };
    ...
    // نجرّب أول مرشّحين بس (زر الدخول الحقيقى غالباً أول واحد بعد ترتيب rank) — تقليل عدد الإرسالات
    for (const btn of cands.slice(0, 2)) {
      try { btn.click(); } catch (e) {} await sleep(1600);
      if (!onLogin()) { log('LOGIN OK'); okDone(); return; }
      if (stopAfterSubmit()) return;   // ظهرت رسالة رفض/قفل → تهدئة وخروج فورى
      realClick(btn); await sleep(1800);
      if (!onLogin()) { log('LOGIN OK'); okDone(); return; }
      if (stopAfterSubmit()) return;
    }
    // كل المحاولات فى النداء ده فشلت من غير رسالة رفض صريحة → نعدّها محاولة فاشلة واحدة
    bumpFail();
    if (_fails >= 2) startLoginCooldown('محاولتين فشلوا بدون رسالة');
    else log('STILL on login. (محاولة فاشلة رقم ' + _fails + ')');
  }
  ...
  log('TE FCC + WFM + OSS + SubInfo v3.0.3 loaded on', location.host);
