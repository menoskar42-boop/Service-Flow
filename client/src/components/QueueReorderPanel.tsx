...
export function QueueReorderPanel() {
  ...
  // ونطاقه = **الباتشات الظاهرة فى القايمة دى بس** (اللى ليها مهام نشطة) — مش أى
  // باتش قديم اتلغى من أسابيع، عشان مايفتحش عشرات الباتشات اللى المستخدم مش شايفها.
  const [requeuingAll, setRequeuingAll] = useState(false);
  const stuckCount = [...priorityRows, ...rows].filter((b) => isStuck(b)).length;
  const shownCount = new Set([...priorityRows, ...rows].map((b) => b.batchId)).size;
  const requeueAll = async () => {
    const scope = `هيتعاد تشغيل الـ ${shownCount} باتش الظاهرين فى القايمة دى بس (الباتشات القديمة اللى خلصت أو اتلغت مش هتترجع).`;
    const msg = stuckCount > 0
      ? `فيه ${stuckCount} باتش باين إنه عالق.\n${scope}\nالمهام الواقفة فيهم (تحت التنفيذ + الملغاة) هترجع للطابور من أول وجديد.\nاللى اتنفّذ فعلاً مش هيتعاد، والموقوف مؤقتاً هيفضل موقوف. تأكيد؟`
