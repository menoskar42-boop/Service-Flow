...
interface BoxBrokenRejectionsReportProps {
  orders: Order[];
}

export function BoxBrokenRejectionsReport({ orders }: BoxBrokenRejectionsReportProps) {
  ...
  const boxBrokenOrders = orders.filter(
    (o) =>
      (o.status === ORDER_STATUS.NOT_FEASIBLE || o.status === ORDER_STATUS.EXTERNAL_NOT_FEASIBLE) &&
      o.rejectionReason === REJECTION_REASONS.BOX_BROKEN &&
      o.boxNumber
  );

  const rows = boxBrokenOrders.map((o) => {
    const lookupCabin =
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "متعذرات بوكس معطل");
    XLSX.writeFile(wb, "box-broken-rejections.xlsx");
  };
      ...
      <Card className="overflow-hidden shadow-sm border-0 bg-white">
        <div className="p-4 border-b space-y-3" dir="rtl">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="font-semibold text-base">متعذرات بوكس معطل</h3>
