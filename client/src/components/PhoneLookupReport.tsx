...
export function PhoneLookupReport() {
  ...
  //  - السوبر أدمن + الأدمن (ومنه مدير السنترال=admin) + الشئون الخارجية (ومنها مهندس
  //    الكوابل=external) يعملوا على أى خط.
  //  - **الفنى: خطوطه هو بس** (ownedByMe = خط فى كباينه، أو خط زميل هو مغطّيه بتصريح
  //    وعليه عطل على الشاشة/اتنظّم النهاردة).
  //  - غير كده → مايتسمحش.
  //
  // ⚠️ متحوّلش دى لـ true. اتغيّرت مرة لـ `canUseTools = true` عشان طلب «القياس يظهر
  // لكل المستخدمين فى بحث برقم التليفون» — وده كسر القاعدة: الفنى محمد بقى يقدر يقيس
  // خط تابع لحسن عبد الفتاح. الطلب ده كان معناه «القياس فى صفحة البحث مش فى التقارير»
  // (شوف useSpeedToolsVisible)، **مش** إلغاء قيد خطوط الفنى.
  const canUseTools =
    isSuper ||
  ...
  // على «نفس الكابينة» الكود القديم افتراضى = 11-2-26-26 (كابينة مختلفة عن الحالية).
  // لو الكابينة الحالية نفسها = 11-2-26-26 (فيحصل تطابق) بنستخدم 11-2-26-01 بدالها.
  // (الكود ده كان 11-2-76-01 قبل ما الكابينة تاخد كودها الجديد.)
  const SAME_CAB_DEFAULT_OLD = "11-2-26-26";
