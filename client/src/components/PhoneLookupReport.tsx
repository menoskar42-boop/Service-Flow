...
export function PhoneLookupReport() {
  ...
  // أزرار القياس/رفع السرعة/الإيقاف هنا **لكل المستخدمين** اللى بيوصلوا للصفحة دى
  // (التاب نفسه مقصور على الأدمن/الفنى/الشئون الخارجية/مسئول البيانات — المبيعات
  // مابتوصلهاش أصلاً). ده رقم **واحد** قدامك، مش باتش، فمفيش خطر على طابور التنفيذ.
  //
  // ⚠️ الفرق عن الأول: كان مسئول البيانات مايشوفهاش خالص، والفنى يشوفها لخطوط
  // منطقته بس (ownedByMe). القاعدة اتوحّدت: القياس من «بحث برقم التليفون» للكل،
  // ومن **التقارير** (باتشات) للسوبر أدمن بس — شوف useSpeedToolsVisible.
