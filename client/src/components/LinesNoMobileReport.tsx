...
import { ChevronRight, ChevronLeft, Loader2, Phone, Check, X } from "lucide-react";
...
export function LinesNoMobileReport() {
  ...
  const queryKey = ["/api/phone-lines/no-mobile", central, cabin, box, phoneFrom, phoneTo, search, page];
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/phone-lines/no-mobile?${params}`, { credentials: "include" });
  ...
  const saveMobile = async (fullPhone: string) => {
    ...
    try {
      ...
      // الرقم بقى له موبايل → مايتحققش شرط التقرير (بدون موبايل) بعدها، فنعيد الجلب فيختفى الصف
  ...
  const handleExport = async () => {
    const res = await fetch(`/api/phone-lines/no-mobile?${buildParams()}`, { credentials: "include" });
  ...
  const handleExportPDF = async () => {
    ...
    printTablePDF({
      title: "تقرير أرقام بدون رقم موبايل",
    ...
    <div className="space-y-4" dir="rtl">
      <Card className="overflow-hidden shadow-sm border-0 bg-white">
        <div className="p-4 border-b flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="font-semibold text-base">أرقام بدون رقم موبايل</h3>
            {data && (
          ...
          <>
            <div className="overflow-x-auto">
              <Table className="text-right text-sm" dir="rtl">
                ...
                <TableBody>
                  {data?.data.length === 0 ? (
                    <TableRow><TableCell colSpan={13} className="text-center py-10 text-muted-foreground">مفيش أرقام — كل الأرقام فى هذا النطاق عندها موبايل مسجّل</TableCell></TableRow>
                  ) : data?.data.map((r, idx) => (
