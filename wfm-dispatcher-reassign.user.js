...
(function () {
  ...
  async function doReassign(code, prevDlg) {
    const dlg = await waitFor(() => { const d = reassignDialog(); return (d && d !== prevDlg) ? d : null; }, 20000);
    if (!dlg) {
      banner("❌ نافذة Re-assign مافتحتش.", "#c62828");
      logln("   🔎 الضغط على Re-assign مافتحش النافذة — مفيش نافذة فيها حقل تاريخ وزر Assign.");
      return false;
    }
    logln("🪟 اتفتحت نافذة Re-assign (متأكد: فيها حقل تاريخ وزر Assign).");
  ...
  async function runFlow(serviceId) {
    ...
    try {
      ...
      let opened = false, prevReassignDlg = null;
      for (let i = 0; i < candidates.length; i++) {
        ...
        // نسجّل أى بقايا نافذة Re-assign موجودة قبل الضغط — عشان نفرّقها عن الجديدة
        prevReassignDlg = MODE === "reassign" ? reassignDialog() : null;
        if (prevReassignDlg) logln("   ℹ️ فيه بقايا نافذة Re-assign قديمة — هستنى نافذة جديدة غيرها.");
        fireClick(item);
      ...
      if (MODE === "reassign") {
        const ok = await doReassign(WORKER, prevReassignDlg);
