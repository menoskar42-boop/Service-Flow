...
(function () {
  ...
  /* ===== منع الشاشة/الجهاز من النوم أثناء القياس =====
     لأن لما الجهاز بينام/يدخل screensaver بيفصل القياس. بنستخدم طريقتين:
     1) Wake Lock API (يشتغل على HTTPS فقط — AXON على HTTP فغالباً يسقط للـ fallback).
     2) Fallback يشتغل على HTTP: فيديو صغير شغّال (من canvas stream) — تشغيل الميديا
        بيمنع الـ screensaver/sleep على Chrome/Edge. */
  (function keepScreenAwake() {
    // (1) Wake Lock API — أفضل طريقة لو الصفحة HTTPS
    let wl = null;
    const acquireWL = async () => {
      try { if (navigator.wakeLock && !wl) { wl = await navigator.wakeLock.request("screen"); wl.addEventListener("release", () => { wl = null; }); console.log("🔆 Wake Lock نشط"); } } catch (e) {}
    };
    acquireWL();
    document.addEventListener("visibilitychange", () => { if (!document.hidden) acquireWL(); });

    // (2) Fallback على HTTP: فيديو صغير شغّال (2×2px) من canvas stream — يمنع النوم
    try {
      ...
      if (stream) {
        ...
        const mount = () => { (document.body || document.documentElement).appendChild(v); const p = () => v.play().catch(() => {}); p(); document.addEventListener("visibilitychange", () => { if (!document.hidden) p(); }); };
