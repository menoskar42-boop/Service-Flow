...
9. **Whenever ANY Tampermonkey/userscript is changed, ALWAYS do BOTH of the following in the same turn:**
   ...
   - (b) **Attach the actual `.js` file to the reply (SendUserFile) AND output the ENTIRE updated script in the chat reply** (the full file, not just the modified section). The user prefers downloading the file directly as `.js`; the inline copy is the fallback. **Never send only the changed section** — always the whole file, both ways.
